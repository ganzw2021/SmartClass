    ...
    <template v-else>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
          <div class="text-sm font-bold text-slate-600">作业提交率列表</div>
          <div class="text-xs text-slate-400">共 {{ tableData.length }} 条记录</div>
        </div>

        <div v-if="tableData.length === 0" class="text-center py-16 text-slate-400 text-sm">暂无作业数据</div>
        ...
        <div v-else class="divide-y divide-slate-50">
          <div v-for="(row, index) in tableData" :key="row.id"
            ...
            <!-- 序号 -->
            <div class="w-8 h-8 rounded-full bg-gradient-to-br from-pink-500 to-purple-500 text-white flex items-center justify-center text-sm font-bold shrink-0">
              {{ index + 1 }}
            </div>

            <!-- 作业信息 -->
            <div class="flex-1 min-w-0">
              <div class="font-medium text-slate-800 truncate">{{ row.title }}</div>
            ...
            <div class="flex items-center gap-6 text-sm">
              <div class="text-center">
                <div class="text-slate-400 text-xs">应交</div>
                <div class="font-medium text-slate-600">{{ row.total_stu }}</div>
              </div>
              <div class="text-center">
                <div class="text-slate-400 text-xs">已交</div>
                <div class="font-medium text-emerald-600">{{ row.sub_cnt }}</div>
              </div>
              <div class="text-center">
                <div class="text-slate-400 text-xs">平均分</div>
                <div class="font-medium text-amber-600">{{ row.avg_score != null ? row.avg_score : '—' }}</div>
              </div>
            </div>
